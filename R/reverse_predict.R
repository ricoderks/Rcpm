...
#'
#' @param model egression model of class \code{lm}
#' @param new_y the new y values, should be a vector
...
reverse_predict <- function(model, new_y, level = 0.95) {
  ...
  x0 <- (mean(new_y) - stats::coef(model)[1]) / stats::coef(model)[2]
  # remove the names
  names(x0) <- NULL
  ...
  sx0 <- syx / stats::coef(model)[2] * sqrt((1 / length(new_y)) + (1 / length(model$model$y)) + ((mean(new_y) - mean(model$model$y))^2 / (stats::coef(model)[2]^2 * sum((model$model$x - mean(model$model$x))^2))))
  names(sx0) <- NULL
  
  # obtain 95% confidence interval two tailed t-statistic for n-2 degrees of freedom
  tv <- stats::qt(p = (1 - (1 - level)/2), 
                  lower.tail = TRUE, 
