...
context("summary-fit-data: input check")
...
test_that("Is input correct data frame", {
  expect_error(summary_fit_data(my_matrix), "mydata needs to be a data frame!")
  expect_error(summary_fit_data(sumfit_wrong), "mydata should contain only the column names PC, R2cum and Q2cum!")
})


context("summary-fit-data: output check")
