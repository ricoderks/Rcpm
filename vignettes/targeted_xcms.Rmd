...
vignette: >
  %\VignetteIndexEntry{Targeted analysis with XCMS}
...
Sometimes more peaks are retrieved and now we need something to get the correct peaks out of this 
table. Jan Stanstrup wrote a function for this `closet_match`. It finds highest intensity peak 
within a certain retention time window. Is this maybe interesting to use. In DA I often try to find 
the peak which has the smallest deviation in retention time from the standard.

```{r best_matching, eval=FALSE}
...
It is not finished here. It still needs some work. 

Maybe retrieve the EIC's for some nice plotting or statistics on the peaks.